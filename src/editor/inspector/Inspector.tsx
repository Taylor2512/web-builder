...
export default function Inspector() {
  ...
  return <InspectorPanel key={node.id} node={node} />
}

function InspectorPanel({ node }: { node: BuilderNode }) {
  ...
  const [bindingsDraft, setBindingsDraft] = useState(() => JSON.stringify(node.bindings ?? [], null, 2))
